...
const pose = (t, x, yaw = 0) => ({
  t, position: { x, y: x * 2, z: x / 2 }, orientation: { yaw },
});
...
test("interpolates irregular timestamps and takes the short yaw arc", () => {
  const samples = [pose(0, 0), pose(2, 2, Math.PI * 0.9), pose(6, 6, -Math.PI * 0.9)];
...
test("seeks long recordings without scanning every pose", () => {
  let reads = 0;
  const samples = new Proxy(Array.from({ length: 100000 }, (_, i) => pose(i, i)), {
    get(target, key) {
